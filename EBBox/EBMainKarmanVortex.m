...
% dt_max = 1e-3;
dt_max = 5e-4;
dt = min([0.25*dh^2/nu, cfl*dh/UIn, dt_max]);
...
while (time<max_time && step<max_step)
    ...
    if (mod(step,100)==0)
        ...
        if (1)
            ...
            xdirf = rho * ebvof_macx .* (ucorr + diffu);
            ydirf = rho * ebvof_macy .* (vcorr + diffv);
            ...
            legend('Cd (stress)', 'Cd (direct)');
            title(['Cd coef. ', num2str(hydro_fx/(0.5*rho*UIn^2*cylinder_D))]);
