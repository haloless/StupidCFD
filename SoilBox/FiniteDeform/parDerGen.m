...
if abs(eP(1))<tol && abs(eP(2))<tol && abs(eP(3))<tol
	...
	L = Is;
	error('not implemented for zero eigenvalues');
elseif abs(eP(1)-eP(2))<tol && abs(eP(1)-eP(3))<tol
	% 3 equal eigenvalues
	L = yd(1) .* Is;
	error('not implemented for equal eigenvalues');
elseif abs(eP(1)-eP(2))<tol || abs(eP(2)-eP(3))<tol || abs(eP(1)-eP(3))<tol
	...
	error('not implemented for 2 equal eigenvalues');
else
	...
	Ib = [...
	y(1)^2, (2)^2,y(7)^2,y(1)*y(2),y(2)*y(7),y(1)*y(7);
