...
Re = 10;
% Re = 200;
nu = max(R0*Omega0,R1*Omega1) * R0 / Re;
...
%
eb_umac0 = zeros(nx+3,ny+2);
for i = 1:nx+3
for j = 1:ny+2
    if sqrt(edgexs(i)^2+cellys(j)^2) < (R0+R1)*0.5
        eb_umac0(i,j) = -Omega0 * cellys(j);
    end
end
end
%
eb_vmac0 = zeros(nx+2,ny+3);
for i = 1:nx+2
for j = 1:ny+3
    if sqrt(cellxs(i)^2+edgeys(j)^2) < (R0+R1)*0.5
        eb_vmac0(i,j) = Omega0 * cellxs(i);
    end
end
end
...
if (1)
    for i = 1:nx+3
    for j = 1:ny+2
        [frac,uint,vint] = EBSampleCellInfo(edgexs(i),cellys(j),dx,dy);
        ebvof_macx(i,j) = frac;
        eb_umac(i,j) = uint;
    end
    end
    for i = 1:nx+2
    for j = 1:ny+3
        [frac,uint,vint] = EBSampleCellInfo(cellxs(i),edgeys(j),dx,dy);
        ebvof_macy(i,j) = frac;
        eb_vmac(i,j) = vint;
    end
    end
...
if (1)
    ...
    if (1)
        ...
        for j = 1:ny+2
            ...
            if R0<r && r<R1
                pp(i,j) = kk^2 * (0.5*r^2 - 0.5*R1^4/r^2 - R1^2*log(r^2));
            end
        end
        end
    end
...
umac = (1.0-ebvof_macx).*umac + eb_umac;
vmac = (1.0-ebvof_macy).*vmac + eb_vmac;
% time stepping
cfl = 0.25;
% dt_max = 1e-3;
% dt_max = 5e-3;
...
LapDiag = zeros(nx,ny);
LapUmac = zeros(nx+1,ny);
LapVmac = zeros(nx,ny+1);
if (0) 
    LapUmac = 1.0 - ebvof_macx(2:nx+2,2:ny+1);
    LapVmac = 1.0 - ebvof_macy(2:nx+1,2:ny+2);
else
    for i = 2:nx+2
    for j = 2:ny+1
        LapUmac(i-1,j-1) = EBSampleFaceFrac(0, edgexs(i),cellys(j), dx,dy);
    end
    end
    for i = 2:nx+1
    for j = 2:ny+2
        LapVmac(i-1,j-1) = EBSampleFaceFrac(1, cellxs(i),edgeys(j), dx,dy);
    end
    end
...
while (time<max_time && step<max_step)
    ...
    if (1)
        ...
        if (0)
            [Du,Dv] = EBVelocityDiffusion(umac,vmac, ...
            eb_umac0,eb_vmac0,ebls_umac,ebls_vmac, ...
            nx,ny,dx,dy);
        end
        ...
        % ustar1 = (1.0-ebvof_macx).*ustar + ebvof_macx.*eb_umac;
        % vstar1 = (1.0-ebvof_macy).*vstar + ebvof_macy.*eb_vmac;
        % ustar = (1.0-ebvof_macx).*ustar;
        % vstar = (1.0-ebvof_macy).*vstar;
        % ustar(2:nx+2,2:ny+1) = LapUmac .* (ustar1(2:nx+2,2:ny+1) ...
        % - ebvof_macx(2:nx+2,2:ny+1).*eb_umac(2:nx+2,2:ny+1));
        % vstar(2:nx+1,2:ny+2) = LapVmac .* (vstar1(2:nx+1,2:ny+2) ...
        % - ebvof_macy(2:nx+1,2:ny+2).*eb_vmac(2:nx+1,2:ny+2));
        % ustar(2:nx+2,2:ny+1) = (ustar1(2:nx+2,2:ny+1) ...
        % - ebvof_macx(2:nx+2,2:ny+1).*eb_umac(2:nx+2,2:ny+1));
        % vstar(2:nx+1,2:ny+2) = (vstar1(2:nx+1,2:ny+2) ...
        % - ebvof_macy(2:nx+1,2:ny+2).*eb_vmac(2:nx+1,2:ny+2));
        % ustar(2:nx+2,2:ny+1) = LapUmac .* ustar1(2:nx+2,2:ny+1);
        % vstar(2:nx+1,2:ny+2) = LapVmac .* vstar1(2:nx+1,2:ny+2);
        % ustar = ustar1;
    ...
    if (1)
        ...
        rhs = EBPPEAdaptedRhs(ustar,vstar, ...
    ...
    if (1)
        ...
        sol = zeros(nx*ny,1);
        [ret,sol] = EBPPESolver(nx,ny,dx,dy, LapDiag,LapUmac,LapVmac, ...
        sol, rhs, method, params);
        if (ret ~= 0) 
            error(['RET=',int2str(ret)]);
        end
    end
