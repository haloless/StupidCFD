...
[Xumac,Yumac] = ndgrid(edgexs,cellys);
[Xvmac,Yvmac] = ndgrid(cellxs,edgeys);
...
ebls_umac = ProbDistFunc(Xumac,Yumac);
ebls_vmac = ProbDistFunc(Xvmac,Yvmac);
%
...
%
eb_usurf = zeros(nx+3,ny+2);
for i = 1:nx+3
for j = 1:ny+2
    if ebls_umac(i,j)>0
        r = sqrt(edgexs(i)^2 + cellys(j)^2);
        if r < 0.5*(R0+R1)
            eb_usurf(i,j) = -Omega0 * (cellys(j)/r*R0);
        else
            eb_usurf(i,j) = -Omega1 * (cellys(j)/r*R1);
        end
    end
end
end
%
eb_vsurf = zeros(nx+2,ny+3);
for i = 1:nx+2
for j = 1:ny+3
    if ebls_vmac(i,j) > 0
        r = sqrt(cellxs(i)^2 + edgeys(j)^2);
        if r < 0.5*(R0+R1)
            eb_vsurf(i,j) = Omega0 * (cellxs(i)/r*R0);
        else
            eb_vsurf(i,j) = Omega1 * (cellxs(i)/r*R1);
        end
    end
end
end
...
if (1)
    ...
    for j = 1:ny+2
        ...
        if r <= R0
            pp(i,j) = 0.5*Omega0^2*(r^2-R0^2) + (pp0 - pp1);
        elseif R0<r && r<R1
...
while (time<max_time && step<max_step)
    ...
    [ustar,vstar] = VelocityInterpIB(ustar,vstar,nx,ny,dx,dy,dt);
