...
% analytical solution
ana = refsol_PresLoadHalfPlane(E, nu, a,L,p);
...
% load gmsh
[groups,nodes] = gmsh.loadMeshFile('presLoadHalfPlane.msh');
...
if 1 % set all displacement
    % 
    ids = groups('right').nodeIds; num = length(ids);
    fixed_nodes_x(end+1:end+num,1) = ids;
    fixed_disp_x(end+1:end+num,1) = zeros(num,1);
    % 
    ids = [groups('bottom').nodeIds; groups('left').nodeIds]; ids = unique(ids); num = length(ids);
    fixed_nodes_x(end+1:end+num,1) = ids;
    ...
    for k = 1:size(ids,1)
        ...
        loaded_nodes(k1) = 1;
        loaded_force(:,k1) = loaded_force(:,k1) - p * [0; 1] .* (len/2);
        
        loaded_nodes(k2) = 1;
        loaded_force(:,k2) = loaded_force(:,k2) - p * [0; 1] .* (len/2);
    end
...
if (1)
    ...
    nodeX(loaded_nodes),nodeY(loaded_nodes),'v');
...
[ana_ux,ana_uy] = ana.disp(nodeX, nodeY);
[ana_sxx,ana_syy,ana_sxy] = ana.stress(nodeX, nodeY);


if 1
    figure;
    trisurf(tri, nodeX,nodeY, nodeSigma(2,:), 'FaceColor','interp');
    view(0, 90); axis equal; xlabel('x'); ylabel('y'); title('sim-syy');
end

if 1
    ylineids = groups('yline').nodeIds;
    [~,yprobe] = sort(nodeX(ylineids));
    yprobe = ylineids(yprobe);
    figure;
    plot(nodeX(yprobe),nodeSigma(2,yprobe),'x-', nodeX(yprobe),ana_syy(yprobe),'.-');
    legend('sim-syy','ana-syy');
end
...
if 1
    ...
    vtk.writeTriMesh('testPresHalfPlane/hoge00.vtk', tri,nodeX,nodeY, dict);
    vtk.writeTriMesh('testPresHalfPlane/hoge01.vtk', tri,nodeX,nodeY, dict);
end
