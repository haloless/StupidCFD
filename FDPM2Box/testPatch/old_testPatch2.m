...
if prob_type == 1
    ...
    for i = 1:nr1
        ...
        if 0
            if 1<i && i<nr1 && 1<j && j<nh1
                rr = 0.2;
                xx = xx + rand(1)*rr*0.5*h0;
                yy = yy + rand(1)*rr*0.5*h0;
            end
        end
...
disp('Begin generate boundary points');
numCons = 0;
consX = [];
consY = [];
consA = [];
...
if prob_type == 1
    ...
    for i = 1:nr1
        if i==1 || i==nr1 || j==1 || j==nh1
            ...
            consA(numCons,1) = 0;
            consB(numCons,1) = 1;
            consC(numCons,1) = fun_v(xx,yy);
        end
    end
    end
end
consPos = [consX,consY];
disp('End generate boundary points');
...
disp('Begin create constraint matrix');
Kcons = sparse(numDofs,numCons);
fcons = zeros(numCons,1);
for k = 1:numCons
    kneigh = cons(k).neigh;
    
    kneighdofx = fdpmNodeDof(kneigh,'x');
    kneighdofy = fdpmNodeDof(kneigh,'y');
    
    Kcons(kneighdofx,k) = Kcons(kneighdofx,k) + consA(k) .* cons(k).Nshape';
    Kcons(kneighdofy,k) = Kcons(kneighdofy,k) + consB(k) .* cons(k).Nshape';
    fcons(k) = fcons(k) + consC(k);
end
disp('End create constraint matrix');
...
if 1
    ...
    if 0
        ...
        fres = frct + fext - fint - fstab;
        rhs = [ fres; fcons ];
        mat = [ Ktan0, Kcons; Kcons', sparse(numCons,numCons) ];
        sol = mat \ rhs;
