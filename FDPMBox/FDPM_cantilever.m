...
if (1) % static load
    ...
    for step = 1:maxStep
        ...
        for iter = 1:maxIter
            ...
            if (1)
                % plot(pos(:,1),pos(:,2),'o');
                plot(pos(:,1),pos(:,2),'o',nodeX,nodeY,'x');
                axis equal;
                title(['outer:',int2str(step),'/inner:',int2str(iter)]);
                drawnow;
                
                if (do_makeavi)
                    Mov(totalCount) = getframe(gcf);
                end
            end
            ...
            disp(['Jmax=',num2str(max(nodeJ)),';Jmin=',num2str(min(nodeJ))]);
    ...
    if(do_makeavi)
        movie2avi(Mov, 'cantilever.avi', ...
            'compression','None', 'fps', 6);
    end
    ...
    figure;
    plot(xpos(:,ny/2),S11(:,ny/2), xpos(:,ny/2),S21(:,ny/2), ...
    xpos(:,ny/2),S12(:,ny/2), xpos(:,ny/2),S22(:,ny/2));
